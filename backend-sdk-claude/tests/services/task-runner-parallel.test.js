...
describe('task-runner paralelismo', () => {
  ...
  test('createTask com agent envolve prompt em Task tool dispatch', async () => {
    query.mockImplementation(slowQuery(50));

    const task = taskRunner.createTask({
      prompt: 'analise o repo',
      agent: 'example-pattern-mapper',
      maxTurns: 1,
      source: 'test',
    });

    expect(task.agent).toBe('example-pattern-mapper');
    expect(task.originalPrompt).toBe('analise o repo');
    expect(task.prompt).toContain("subagent_type='example-pattern-mapper'");
    expect(task.prompt).toContain('analise o repo');
    expect(task.tags).toContain('agent');
    expect(task.tags).toContain('agent:example-pattern-mapper');

    // Drena a task até terminar pra não logar após o teardown do teste
    // (senão jest acusa "Cannot log after tests are done" e sai com código 1).
    const deadline = Date.now() + 1500;
    while (Date.now() < deadline) {
      const t2 = taskRunner.getTask(task.id);
      if (t2 && (t2.status === 'done' || t2.status === 'error')) break;
      await sleep(50);
    }
  });
});
